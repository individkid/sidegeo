...
Another module, AffTopo/Sculpt.hs, displays polytopes with OpenGL, and allows a user to manipulate them. Sculpt.hs uses a foreign function interface to c code. The first of the following c files implements a thread intraface that communicates between the threads. Microcode is glsl code for graphics engines interfaced to with OpenGl. Common is shared types, utility functions, and mutex queues.
  ...
  * common contains mutex queues are staged to and from thread local queues. command has void function pointer. haskell has event enumeration. console has endline terminated string. process has string pointer. configure has file structure. Command, Event, Kind, Data, Output, Option, File are unique types. CmdChar, CmdInt, HsChar, HsInt are uniquefied types. the basename identifies the consumer of the function and arguments. Cmn prefixed queues in common map onto unprefixed queues in threads. Hs prefixed queues in haskell and Cmd prefixed queues in command map into Cmn queues in common.  
  * main combines -arg arg from argv into one liners to append to Option. process reads from Option or Configure depending on toggle. toggle starts on Option, goes to Configure upon -f, goes to Option upon --yield. configure sends --yield upon eof. Optioner and Configurer are sideband. every --arg has file ident sideband, whether from Optioner or Configurer. configure appends --arg to Configure; others append -arg or --arg to Option. process handles --arg from Configure and -arg from Option directly. process sends --arg from Option through Inject to configure indicated by Optioner. -e arg appends --arg to Option and current -E ident to Optioner. --inject arg from configure appends -arg to Option. -e -* and --inject -* in process are errors. --arg in console is converted to -e arg. errors in files cause ignore of Configure with Configurer from that file. errors from console print message and are dropped.  
