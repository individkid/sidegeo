...
Another module, AffTopo/Sculpt.hs, displays polytopes with OpenGL, and allows a user to manipulate them. Sculpt.hs uses a foreign function interface to c code. The first of the following c files implements a thread intraface that communicates between the threads. Microcode is glsl code for graphics engines interfaced to with OpenGl. Common is shared types, utility functions, and mutex queues.
  ...
  * common mutex queues are staged to and from thread local queues. each thread has a function type. command has void function pointer. haskell has event enumeration. console has endline terminated string process has string pointer. configure has file structure. some threads have function arguments. types unique to a thread are in queues with basename of the type. for example, Command, Event, Kind, Data, Output, Option, File are unique types. common types are made unique by prefixing compressed thread name. for example, CmdChar, CmdInt, HsChar, HsInt are uniquefied types. thus, the basename identifies the consumer of the function and arguments. the function and arguments are staged to queues, without prefix, local to the thread, and from queues, with Cmn prefix, in common. thus Cmn queues in common map onto barename queues in threads. request function and arguments are staged in the requester from queues prefixed with compressed requester thread name. for example, before mutexing into CmnCommand CmnCmdChar CmnCmdInt, haskell thread stages into HsCommand HsCmdChar HsCmdInt. thus Hs queues in haskell map into Cmn queues in common.  
