...
side0_cospace6 (Side0 st) = (Cospace6 dt bs rs ss) where
 ...
 (Sides0 ss) = side0_sides0 (Side0 st)
 (Dual0 dt) = dual2_dual0 (Dual2 st bs rs ss)
side0_duals0 (Side0 st) = Duals0 (setMap f rs) where
 f r = fromSet (g r) bs
 g r b = sub2 st b r
 (Bounds0 bs) = side0_bounds0 (Side0 st)
 (Regs0 rs) = side0_regs0 (Side0 st)
...
-- TODO allow given map to not include all vertices
...
classify1 :: Plane0 -> Side0
classify1 (Plane0 cb) = undefined
...
{-
display :: Space -> Friendly
poke :: Space -> Arrow -> [Pierce]
scratch :: Point -> Arrow -> Transform
apply :: Space -> [Color] -> Transform -> Space
pierce :: Pierce -> (Facet,Point)
facet :: Facet -> (Region,Region,Color,Color)
add :: Space -> Facet -> Space
subtract :: Space -> Facet -> Space
refine :: Space -> Pierce -> Space
-}